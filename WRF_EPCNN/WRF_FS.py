...
def read_files(file_name):
    ...
    known = pd.read_csv(file_name+'_known.csv', index_col=0)
    unknown = pd.read_csv(file_name+'_unknown.csv', index_col=0)

    y = pd.read_csv(file_name+'_y.csv', index_col=0)
...
def WRF_sff(known, unknown, y):
    ...
    clfx = RandomizedSearchCV(
        ...
        cv=5, n_jobs=-1, iid=False, random_state=4489)
